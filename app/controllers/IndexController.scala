...
class IndexController @Inject() (
  ...
  private def populateIndividualUserAnswers(
    ...
    } yield Redirect(controllers.individual.routes.CheckDetailsController.onPageLoad())

  private def populateBusinessUserAnswers(personalRep: BusinessPersonalRep, userAnswers: UserAnswers): Future[Result] =

    for {
      updatedAnswers <- Future.fromTry(businessExtractor(personalRep, userAnswers))
      _              <- repository.set(updatedAnswers)
    } yield Redirect(controllers.business.routes.CheckDetailsController.onPageLoad())
