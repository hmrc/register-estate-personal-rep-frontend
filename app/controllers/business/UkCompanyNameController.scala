...
class UkCompanyNameController @Inject()(
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = actions.authWithData {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(UkCompanyNamePage) match {
        case None => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = actions.authWithData.async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        value => {
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(UkCompanyNamePage, value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(UkCompanyNamePage, mode, updatedAnswers))
        }
      )
  }
}
