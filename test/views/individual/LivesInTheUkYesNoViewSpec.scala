...
class LivesInTheUkYesNoViewSpec extends YesNoViewBehaviours {

  val prefix = "individual.livesInTheUkYesNo"
  val name: Name = Name("First", None, "Last")

  val form: Form[Boolean] = new YesNoFormProvider().withPrefix(prefix)

  val view: LivesInTheUkYesNoView = viewFor[LivesInTheUkYesNoView](Some(emptyUserAnswers))

  def applyView(form: Form[_]): HtmlFormat.Appendable =
    view.apply(form, NormalMode, name.displayName)(fakeRequest, messages)

  "LivesInTheUkYesNo View" must {
